...
export const SurahCard = ({ surah }) => {
  ...
  return (
    <Link
      to={`/quran/${number}`}
      className="bg-white dark:bg-slate-900 border border-slate-200/50 dark:border-slate-800/80 rounded-3xl p-5 hover:shadow-md hover:border-brand-emerald-200/60 dark:hover:border-brand-emerald-950/60 transition-all duration-300 flex items-center justify-between group"
    >
      <div className="flex items-center gap-4">
        {/* Surah Number Icon */}
        <div className="relative w-12 h-12 flex items-center justify-center flex-shrink-0">
          {/* Islamic Star Shaped BG using inline SVG */}
        ...
        <div className="flex flex-col min-w-0">
          <span className="font-bold text-slate-855 dark:text-white group-hover:text-brand-emerald-600 dark:group-hover:text-brand-emerald-400 transition-colors">
            {englishName}
          </span>
      ...
      <div className="text-right flex flex-col items-end gap-1.5">
        <span className="arabic-text font-semibold text-lg text-slate-900 dark:text-white leading-none">
          {name}
        </span>
